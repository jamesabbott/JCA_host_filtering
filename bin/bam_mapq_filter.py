...
def main():
	...
	for read in sam.fetch(until_eof=True):
		if read.is_unmapped:
			outsam.write(read)
		elif read.is_paired and read.mapping_quality < args.mapq:
			outsam.write(read)
	...
	copyfile('{}/{}_unmapped_1.fq.gz'.format(tmpdir,sample),
		'filtered_fastq/mapq_{}/{}_unmapped_1.fq.gz'.format(args.mapq,sample))
	copyfile('{}/{}_unmapped_2.fq.gz'.format(tmpdir,sample),
		'filtered_fastq/mapq_{}/{}_unmapped_2.fq.gz'.format(args.mapq,sample))
