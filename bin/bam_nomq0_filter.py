...
def main():
	...
	parser=argparse.ArgumentParser("Extracts unaligned reads, and those with a MAPQ of 0 from aligned reads")
	...
	try:
		os.mkdir('filtered_fastq/nomq0'.format(args.mapq))
	...
	copyfile('{}/{}_unmapped_1.fq.gz'.format(tmpdir,sample),
		'filtered_fastq/nomq0/{}_unmapped_1.fq.gz'.format(sample))
	copyfile('{}/{}_unmapped_2.fq.gz'.format(tmpdir,sample),
		'filtered_fastq/nomq0/{}_unmapped_2.fq.gz'.format(sample))
