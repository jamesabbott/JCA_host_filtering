...
mkdir -p filtered_fastq/morex3_unaligned_single

cp -rv bwa_alignments/${sample}* $TMPDIR/
samtools fastq -@ 24 -f 4 -s /dev/null -N -1 $TMPDIR/${sample}_unmapped_single_1.fq.gz -2 $TMPDIR/${sample}_unmapped_single_2.fq.gz \
	-0 /dev/null $TMPDIR/${sample}.sorted.bam

cp -v ${TMPDIR}/${sample}_unmapped*.gz filtered_fastq/morex3_unaligned_single
